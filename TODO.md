...
> 最后整理：2026-08-12。本文档是根目录唯一的项目待办来源；实现细节发生变化时同步更新本文件。
...
- [x] 2026-08-12 新增默认“知识云”皮肤，对齐 `wiki-cloud` 的系统蓝、半透明应用栏、浅灰页面底、柔和分隔线与圆角控件；保留构案、浅色和深色主题供切换，并保持既有用户主题偏好兼容。

## 已完成的远程知识集成

- [x] 2026-08-12 构案桌面端接入 `wiki-cloud` 只读 provider：设置页支持服务地址、Workspace、API Key、知识库范围和检索策略；API Key 仅由 Tauri 后端写入系统凭据库，浏览器端禁用远程连接且项目缓存不保存密钥。
- [x] 2026-08-12 右侧资料面板支持在本地知识与 `wiki-cloud` 间切换，远程命中可预览并显式加入 Agent 上下文，保留 knowledge base、document、chunk、版本、定位和来源 URI 以支持引用追溯。
...
目标：以构案确定性状态机编排 OpenCode HTTP 标题 Worker；修改文档直接按 H2-H6 目标串行分析和写入，新建文档仅先生成轻量目录，并通过范围校验、hash 和回滚保证文件安全。

### OpenCode HTTP Server 与会话

- [x] Rust 托管仅绑定 `127.0.0.1` 的 `opencode serve --pure`，使用随机端口、内存 Basic Auth、健康检测、精确 PID 关闭和一次崩溃重启。
- [x] 修改文档直接为每个标题创建独立 HTTP session；新建文档仅额外创建轻量目录生成 session，支持 SSE、状态查询和 abort。
- [x] 模型列表来自 OpenCode `/provider`，支持在下拉框内按供应商、模型名称和模型 ID 搜索；不向 OpenCode 同步构案 API Key。
- [x] 运行面板以单一 Agent 对话形式按 session 实时展示输入、OpenCode 可见思考、工具调用、输出和错误；完整实时流不写入任务持久化记录。

### 领域模型与章节解析

- [x] 独立定义 `LongWritingMode`、任务/章节状态、`OutlinePlan`、`ChapterJob` 和 `ConsistencyIssue`，不混入 `Project` 或普通 Agent 会话。
- [x] H1 仅作根节点，H2-H6 任意标题子树均可成为任务；稳定解析忽略代码围栏内井号，支持重复标题、父级覆盖后代和同名出现序号。
- [x] 章节树支持搜索、全选/取消全选、全部展开/收起和部分选择状态。
- [x] 为任务记录基础/当前全文 hash，为章节记录原文/提交目标 hash；API Key 不进入任务记录。
- [ ] 补充超大文档的分段摘要预算基准和压力测试。

### 目录审核与串行 Worker

- [x] 任务模式只保留“修改文档”和“新建文档”。
- [x] 修改文档跳过 Coordinator 和预规划阶段，按用户选择的 H2-H6 标题顺序直接创建 Worker。
- [x] 新建文档仅由轻量目录生成器输出可审核 Markdown 目录；确认后按 H2 顺序创建正文 Worker。
- [x] 每个 Worker 都接收正式文件绝对路径、唯一目标标题范围、用户原始总指令和相关引用资料，不重复注入全文子树或 Document Bible。
- [x] Worker 在同一轮重新读取正式文件、直接编辑并保存；不先输出编辑计划，所有目标严格串行执行。
- [x] 启动时冻结 `project.contextSourceRefs` 引用资料；标题任务只接收相关片段，不自动检索未引用资料。
- [x] 允许修正当前目标标题的明显错别字或病句，同时冻结子标题文本、层级、父子关系和目标范围外内容。
- [x] 支持暂停、继续、停止、失败重试、请求取消和越界修改人工确认。
...
- [x] OpenCode 直接编辑后验证目标子树外内容与冻结目录签名；仅在磁盘仍等于 session 输出 hash 时回滚，外部修改进入冲突。
- [x] 一次性删除旧架构任务记录及清单可证明归属的备份正文/JSON；保留手工、无清单和无法证明归属的备份。
- [ ] 增加新架构历史备份的用户可控保留和清理策略。

### UI、一致性检查与验收

- [x] Agent 区提供独立“长任务”入口和启动向导：Server 状态、模式、总指令、H2-H6 范围、当前引用资料摘要、OpenCode 模型及并发数。
- [x] 增加“新建文档”模式：由轻量目录生成器生成可审核目录；确认后按 H2 标题顺序串行写作。
- [x] 目录阶段仅展示可编辑的 Markdown 目录；确认后才启动正文生成。
- [x] 运行界面展示总进度和每章排队、运行、校验、写入、完成、失败状态，并提供暂停、继续、停止、单章重试和恢复原文。
- [x] 运行界面提供可恢复的执行动态、章节展开预览、原文/AI 稿差异、摘要/事实/术语/待确认项，以及编辑器章节定位。
- [x] 每个标题写入后立即执行本地范围与标题结构检查；越界修改自动回滚并等待用户确认，不再追加模型级全文一致性检查。
- [ ] 使用真实 OpenCode 1.18.11 和至少 9 个 H2-H6 目标完成暂停、恢复、崩溃重连、越界回滚、冲突和 DOCX 导出手工验收。
- [ ] 补齐 Tauri 集成级崩溃注入测试（提交标记后、原子替换前后）。

## 长任务模式暂不包含

- 模型递归创建自治 subagent，或脱离构案状态机访问其他文件。
- Browser/Tauri 等价运行；OpenCode HTTP 长任务仅桌面端。
