...
目标：以构案确定性状态机编排 OpenCode HTTP Coordinator 与 H2-H6 子 session；并行分析、串行直接写入正式 Markdown，并通过范围校验、hash 和回滚保证文件安全。

### OpenCode HTTP Server 与会话

- [x] Rust 托管仅绑定 `127.0.0.1` 的 `opencode serve --pure`，使用随机端口、内存 Basic Auth、健康检测、精确 PID 关闭和一次崩溃重启。
- [x] Coordinator 与全部标题子任务统一使用 HTTP session；子 session 通过 `parentID` 关联主 session，支持 SSE、状态查询和 abort。
- [x] 模型列表来自 OpenCode `/provider`，支持在下拉框内按供应商、模型名称和模型 ID 搜索；不向 OpenCode 同步构案 API Key。
- [x] 运行面板按 session 实时展示可见文本、状态、工具名称和错误；不展示隐藏推理或工具输入，完整实时流不持久化。
...
- [x] 子 session 分两阶段执行：分析阶段只读且可限并发，写入阶段重新读取最新正式文件并进入提案级串行队列。
- [x] 启动时冻结 `project.contextSourceRefs` 引用资料；Coordinator 接收预算内摘要，标题任务只接收相关片段，不检索未引用资料或自动联网。
...
- [x] 增加“从零创建完整方案”模式：仅允许空白或 H1-only 的已保存文档，由 Coordinator 生成可审核目录；确认后冻结目录并按所选 H2-H6 子树写作。
