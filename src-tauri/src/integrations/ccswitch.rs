            ...
            if let Ok(value) = serde_json::from_str::<Value>(&text) {
                if let Some(path) = value.get("app_config_dir_override").and_then(Value::as_str) {
                    ...
                    if !path.as_os_str().is_empty() {
                        paths.push(path.join("cc-switch.db"));
                    }
                }
            }
        }
...
fn read_providers(path: &PathBuf) -> Result<Vec<CcSwitchProvider>, String> {
    ...
    for row in rows {
        let (source_id, app_type, name, settings) =
            row.map_err(|error| format!("读取 CCSwitch provider 失败: {error}"))?;
        let Ok(config) = serde_json::from_str::<Value>(&settings) else {
            continue;
        };
        if let Some(provider) = provider_from_value(source_id, app_type, name, config) {
...
fn provider_from_value(
    ...
    let base_url = first_string(&config, base_keys)
        .or_else(|| config_text.and_then(|text| toml_string(text, "base_url")))
        .unwrap_or_default()
        .trim()
        .trim_end_matches('/')
        .to_string();
    let api_key = first_string(&config, key_keys)
        .or_else(|| config_text.and_then(|text| toml_string(text, "api_key")))
        .unwrap_or_default();
    let mut models = Vec::new();
    for path in model_keys {
        push_unique(&mut models, value_at(&config, path));
    }
    ...
    if let Some(text) = config_text {
        push_unique(&mut models, toml_string(text, "default"));
        push_unique(&mut models, toml_string(text, "model"));
    }
...
fn is_chat_protocol(config: &Value) -> bool {
    first_string(config, &[&["api_format"], &["apiFormat"]])
        .map(|v| {
            matches!(
        ...
        .unwrap_or(false)
        || first_string(config, &[&["base_url"], &["baseURL"]])
            .map(|v| v.to_ascii_lowercase().ends_with("/chat/completions"))
            .unwrap_or(false)
}
