...
pub(super) fn normalized_heading_text(value: &str) -> String {
    ...
    if let Some(caps) = Regex::new(r"^\[([^]]+)\]\(#_Toc[^)]*\)$")
        .unwrap()
        .captures(text.trim())
    {
...
fn inferred_level(text: &str) -> Option<usize> {
    ...
    if chapter.is_match(&cleaned) {
        return Some(1);
    }
    let decimal =
        Regex::new(r"^(?:[0-9]+|[一二三四五六七八九十百零〇两]+)(?:[.．][0-9]+)+").unwrap();
    decimal
        .find(&cleaned)
        .map(|m| (m.as_str().matches(['.', '．']).count() + 1).min(6))
}
...
fn candidate(
    ...
    HeadingCandidate {
        id: stable_id("hc", &format!("{id_seed}:{line}")),
        line,
        text: normalized_heading_text(original),
...
pub(super) fn detect_heading_candidates(
    document_id: &str,
    markdown: &str,
) -> (Vec<HeadingCandidate>, Option<usize>, Option<usize>) {
    let lines: Vec<&str> = markdown.lines().collect();
    let toc_re = Regex::new(r"^\s*\[([^]]+)\]\(#_Toc[^)]*\)\s*$").unwrap();
    let toc_entries: Vec<(usize, String, usize)> = lines
        .iter()
        .take(500)
        .enumerate()
        .filter_map(|(i, line)| {
    ...
    let (toc_start, toc_end) = if toc_entries.len() >= 3 {
        let first = toc_entries.first().map(|x| x.0).unwrap_or(0);
        let label = (first.saturating_sub(8)..first)
            .rev()
            .find(|i| normalized_heading_text(lines[*i]).replace(' ', "") == "目录");
        (
            Some(label.unwrap_or(first)),
            toc_entries.last().map(|x| x.0),
        )
    } else {
        (None, None)
    };
    ...
    if let Some(end) = toc_end {
        for (_, toc_title, level) in &toc_entries {
            if let Some((index, line)) = lines.iter().enumerate().skip(end + 1).find(|(i, line)| {
                !by_line.contains_key(i) && normalized_heading_text(line) == *toc_title
            }) {
                by_line.insert(
                    index,
                    candidate(
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        let text = normalized_heading_text(line);
        let blank_before = index == 0 || lines[index - 1].trim().is_empty();
        let blank_after = index + 1 >= lines.len() || lines[index + 1].trim().is_empty();
...
pub(super) async fn resolve_ambiguous_candidates(
    candidates: &mut [HeadingCandidate],
    markdown: &str,
    mut config: ModelConfig,
) -> Option<String> {
    let pending: Vec<_> = candidates
        .iter()
        .filter(|c| !c.selected && c.source == "candidate")
        .take(80)
        .map(|c| {
            let lines: Vec<&str> = markdown.lines().collect();
            let start = c.line.saturating_sub(2);
            let end = (c.line + 3).min(lines.len());
    ...
    if config.model.trim().is_empty() {
        return Some("未配置模型，低置信度标题保留为待确认".into());
    }
    if config.api_key.is_empty()
        && !config.base_url.contains("localhost")
        && !config.base_url.contains("127.0.0.1")
    {
        return Some("API Key 未配置，低置信度标题保留为待确认".into());
    }
    ...
    let result = async {
        ...
        if !response.status().is_success() {
            return Err(format!("模型服务返回 {}", response.status()));
        }
        ...
        let raw = body
            ...
            .trim_end_matches("```")
            .trim();
        serde_json::from_str::<Value>(raw).map_err(|_| "模型未返回有效 JSON".to_string())
    }
    .await;
    match result {
        Ok(value) => {
...
pub(super) fn apply_heading_decisions(
    content: &str,
    decisions: &[HeadingReviewDecision],
    toc_start: Option<usize>,
    toc_end: Option<usize>,
) -> String {
    let selected: BTreeMap<usize, &HeadingReviewDecision> = decisions
        .iter()
        .filter(|d| d.selected)
        .map(|d| (d.line, d))
        .collect();
    let mut output = Vec::new();
    let mark_toc = !content.contains("<!-- knowledge-toc:start -->");
    for (index, line) in content.lines().enumerate() {
        if mark_toc && Some(index) == toc_start {
            output.push("<!-- knowledge-toc:start -->".to_string());
        }
        if let Some(decision) = selected.get(&index) {
            let _candidate_id = &decision.id;
            if markdown_heading(line).is_some() {
                output.push(line.to_string())
            } else {
                output.push(format!(
                    "{} {}",
                    "#".repeat(decision.level.clamp(1, 6)),
                    line.trim_start()
                ))
            }
        } else {
        ...
        if mark_toc && Some(index) == toc_end {
            output.push("<!-- knowledge-toc:end -->".to_string());
        }
    }
