...
fn memory_root(workspace: &WorkspacePaths) -> Result<PathBuf, String> {
    ...
    for kind in VALID_TYPES { fs::create_dir_all(root.join(kind)).map_err(|e| format!("创建记忆目录失败: {e}"))?; }
...
fn validate_type(value: &str) -> Result<String, String> {
    ...
    if VALID_TYPES.contains(&value.as_str()) { Ok(value) } else { Err(format!("不支持的记忆类型: {value}")) }
}

fn validate_status(value: &str) -> Result<String, String> {
    let value = value.trim().to_ascii_lowercase();
    if VALID_STATUSES.contains(&value.as_str()) { Ok(value) } else { Err(format!("不支持的记忆状态: {value}")) }
}
...
fn validate_id(value: &str) -> Result<String, String> {
    ...
    if value.is_empty() || value.len() > 80 || !value.bytes().all(|byte| byte.is_ascii_alphanumeric() || byte == b'-' || byte == b'_') {
...
fn render_entry(entry: &MemoryEntry) -> Result<String, String> {
    let meta = serde_json::to_string(&MemoryFileMeta{id:entry.id.clone(),memory_type:entry.memory_type.clone(),title:entry.title.clone(),confidence:entry.confidence.clone(),status:entry.status.clone(),source_conversation_id:entry.source_conversation_id.clone(),source_message_id:entry.source_message_id.clone(),created_at:entry.created_at,updated_at:entry.updated_at}).map_err(|e| e.to_string())?;
    Ok(format!("<!-- gouan-memory-v{MEMORY_VERSION}: {meta} -->\n\n# {}\n\n{}\n", entry.title, entry.content.trim()))
}
...
fn parse_entry(path: &Path) -> Result<MemoryEntry, String> {
    ...
    let json = first.strip_prefix(&prefix).and_then(|line| line.strip_suffix(" -->"))
        .ok_or_else(|| format!("无效的记忆文件: {}", path.display()))?;
    let meta: MemoryFileMeta = serde_json::from_str(json).map_err(|e| format!("解析记忆元数据失败: {e}"))?;
    let mut lines = raw.lines().skip(1);
    while lines.next().is_some_and(|line| line.trim().is_empty()) {}
    let content = lines.skip_while(|line| line.trim().is_empty()).collect::<Vec<_>>().join("\n").trim().to_string();
    Ok(MemoryEntry{id:meta.id,memory_type:meta.memory_type,title:meta.title,content,confidence:meta.confidence,status:meta.status,source_conversation_id:meta.source_conversation_id,source_message_id:meta.source_message_id,created_at:meta.created_at,updated_at:meta.updated_at})
}
...
fn collect_markdown(dir: &Path, out: &mut Vec<PathBuf>) -> Result<(), String> {
    ...
    for item in fs::read_dir(dir).map_err(|e| format!("扫描记忆目录失败: {e}"))? {
        ...
        if path.is_dir() {
            if path.file_name().and_then(|v| v.to_str()) != Some(".trash") { collect_markdown(&path, out)?; }
        } else if path.extension().and_then(|v| v.to_str()) == Some("md") { out.push(path); }
    }
    Ok(())
}
...
fn reconcile(workspace: &WorkspacePaths) -> Result<Vec<MemoryEntry>, String> {
    ...
    tx.execute("DELETE FROM agent_memory_fts", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM agent_memory", []).map_err(|e| e.to_string())?;
    for path in paths {
        let Ok(entry) = parse_entry(&path) else { continue };
        ...
        tx.execute("INSERT INTO agent_memory_fts(id,title,content) VALUES(?1,?2,?3)", params![entry.id,entry.title,entry.content]).map_err(|e| e.to_string())?;
        entries.push(entry);
    }
...
fn find_entry(workspace: &WorkspacePaths, id: &str) -> Result<MemoryEntry, String> {
    reconcile(workspace)?.into_iter().find(|entry| entry.id == id).ok_or_else(|| format!("找不到记忆: {id}"))
}

fn write_entry(workspace: &WorkspacePaths, input: MemoryWriteInput, actor: &str) -> Result<MemoryEntry, String> {
    let memory_type = validate_type(&input.memory_type)?;
    let title = input.title.trim();
    let content = input.content.trim();
    if title.is_empty() || content.is_empty() { return Err("记忆标题和内容不能为空".into()); }
    if content.len() > 16 * 1024 { return Err("单条记忆不能超过 16KB".into()); }
    let existing = input.id.as_deref().and_then(|id| find_entry(workspace, id).ok());
    let now = now_ms();
    let entry = MemoryEntry {
        id: match input.id { Some(id) => validate_id(&id)?, None => make_id(&format!("{title}:{content}")) },
        memory_type: memory_type.clone(), title: title.to_string(), content: content.to_string(),
        confidence: input.confidence.filter(|v| v == "inferred").unwrap_or_else(|| "confirmed".into()),
        ...
        created_at: existing.as_ref().map(|v| v.created_at).unwrap_or(now), updated_at: now,
    };
    let root = memory_root(workspace)?;
    if let Some(old) = existing.as_ref() {
        for kind in VALID_TYPES { let old_path = root.join(kind).join(format!("{}.md", old.id)); if old_path.exists() { let _ = fs::remove_file(old_path); } }
    }
...
#[tauri::command]
pub fn memory_list(workspace: WorkspacePaths, include_pending: Option<bool>) -> Result<Vec<MemoryEntry>, String> {
    let include_pending = include_pending.unwrap_or(true);
    Ok(reconcile(&workspace)?.into_iter().filter(|entry| entry.status != "archived" && (include_pending || entry.status == "active")).collect())
}

#[tauri::command]
pub fn memory_read(workspace: WorkspacePaths, id: String) -> Result<MemoryEntry, String> { find_entry(&workspace, &id) }

#[tauri::command]
pub fn memory_search(workspace: WorkspacePaths, query: String, limit: Option<usize>) -> Result<Vec<MemoryEntry>, String> {
    reconcile(&workspace)?;
    ...
    let rows = stmt.query_map(params![fts, limit.unwrap_or(8).clamp(1,32) as i64], |row| Ok(MemoryEntry{id:row.get(0)?,memory_type:row.get(1)?,title:row.get(2)?,content:row.get(3)?,confidence:row.get(4)?,status:row.get(5)?,source_conversation_id:row.get(6)?,source_message_id:row.get(7)?,created_at:row.get(8)?,updated_at:row.get(9)?})).map_err(|e| e.to_string())?;
    let found = rows.collect::<Result<Vec<_>,_>>().map_err(|e| e.to_string())?;
    ...
    let rows = fallback.query_map(params![like, limit.unwrap_or(8).clamp(1,32) as i64], |row| Ok(MemoryEntry{id:row.get(0)?,memory_type:row.get(1)?,title:row.get(2)?,content:row.get(3)?,confidence:row.get(4)?,status:row.get(5)?,source_conversation_id:row.get(6)?,source_message_id:row.get(7)?,created_at:row.get(8)?,updated_at:row.get(9)?})).map_err(|e| e.to_string())?;
    rows.collect::<Result<Vec<_>,_>>().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn memory_write(workspace: WorkspacePaths, input: MemoryWriteInput) -> Result<MemoryEntry, String> { write_entry(&workspace, input, "user") }

#[tauri::command]
pub fn memory_propose(workspace: WorkspacePaths, input: MemoryWriteInput) -> Result<MemoryEntry, String> { write_entry(&workspace, MemoryWriteInput{status:Some("pending_review".into()),confidence:Some("inferred".into()),..input}, "agent") }

#[tauri::command]
pub fn memory_accept(workspace: WorkspacePaths, id: String) -> Result<MemoryEntry, String> {
    let old = find_entry(&workspace, &id)?;
    write_entry(&workspace, MemoryWriteInput{id:Some(old.id),memory_type:old.memory_type,title:old.title,content:old.content,confidence:Some("confirmed".into()),status:Some("active".into()),source_conversation_id:old.source_conversation_id,source_message_id:old.source_message_id}, "user")
}
...
pub fn memory_delete(workspace: WorkspacePaths, id: String) -> Result<(), String> {
    ...
    let root = memory_root(&workspace)?;
    let source = root.join(&entry.memory_type).join(format!("{}.md", entry.id));
    if source.exists() { fs::rename(&source, root.join(".trash").join(format!("{}-{}.md", now_ms(), entry.id))).map_err(|e| format!("归档记忆失败: {e}"))?; }
    reconcile(&workspace)?;
...
#[tauri::command]
pub fn memory_rebuild(workspace: WorkspacePaths) -> Result<Vec<MemoryEntry>, String> { reconcile(&workspace) }
...
mod tests {
    ...
    fn workspace() -> (WorkspacePaths, PathBuf) {
        ...
        (WorkspacePaths{root:root.to_string_lossy().into(),history_dir:String::new()},root)
    }
    #[test]
    fn persists_searches_and_archives_memory() {
        let (workspace,root)=workspace();
        let saved=memory_write(workspace.clone(),MemoryWriteInput{id:None,memory_type:"decision".into(),title:"部署平台".into(),content:"使用 Windows Server 2022".into(),confidence:None,status:Some("active".into()),source_conversation_id:None,source_message_id:None}).unwrap();
        assert_eq!(memory_search(workspace.clone(),"Windows Server".into(),None).unwrap().len(),1);
        let path=root.join(".gouan/memory/decision").join(format!("{}.md",saved.id));
        assert!(path.exists());
        let raw=fs::read_to_string(&path).unwrap().replace("Windows Server 2022","Windows Server 2025");
        fs::write(&path,raw).unwrap();
        assert_eq!(memory_rebuild(workspace.clone()).unwrap()[0].content,"使用 Windows Server 2025");
        assert_eq!(memory_search(workspace.clone(),"Server 2025".into(),None).unwrap().len(),1);
